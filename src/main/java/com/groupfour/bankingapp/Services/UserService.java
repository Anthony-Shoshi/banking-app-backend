    ...
    public LoginResponseDTO login(LoginRequestDTO loginRequest) throws AuthenticationException {
        ...
        if (user != null && passwordEncoder.matches(loginRequest.password(), user.getPassword())) {
            ...
            if (customer != null && customer.getStatus() == CustomerStatus.APPROVED) {
                String token = jwtTokenProvider.createToken(user.getFirstName(), user.getLastName(), user.getEmail(), user.getUserId(), user.getRole(), true); // Always set approved to true here
